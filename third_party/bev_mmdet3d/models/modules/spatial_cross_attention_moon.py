...
from .NAT_moon import CrossNATLayer
...
class SpatialCrossAttention(BaseModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if module_type == 'NAT' :
        # adding new query
            query = query.unsqueeze(0).repeat(num_cams,1,1,1)
            ...
            ).view(bs, self.num_cams, 2500, self.embed_dims)
        ...
        **kwargs,
        ...
        if module_type == 'NAT' :
            ...
            if reference_points.shape[-1] == 2:
                ...
                nat_reference_points = reference_points
        ...
        if torch.cuda.is_available() and value.is_cuda:
            ...
            elif module_type =='NAT' :
                _ , q_dim, _ = query.shape
                n_heads = find_smallest_divisor(q_dim)
                NATFunction = CrossNATLayer(input_size=(8,32) , dim= q_dim  , num_heads= n_heads , window_size =7)
                nat_output = NATFunction(query,value)
                output =nat_output.reshape(6,-1,256)

            
            elif module_type =='MSDeform_pytorch' :
